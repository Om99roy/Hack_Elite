...
router.post('/register', async (req, res) => {
  try {
    ...
    const { email, phone, password, fullName, dateOfBirth, gender, emergencyContact, medicalHistory } = value;
...
router.post('/enable-biometric', auth, async (req, res) => {
  try {
    ...
    if (!biometricTemplate) {
      return res.status(400).json({ message: 'Biometric template is required' });
    }
